...
- Метод: `POST /api/v1/proc_image`
- Назначение: анализ изображения автобуса и извлечение метрик (загруженность, кол-во людей, свободные места, свободные входы)
- Тело запроса (JSON):
```json
{
  "image_bytes": "<BASE64 JPEG/PNG>",
  "cam_info": "frontal | gate",
  "cam_num": 1,
  "gate_pos": [1, 2],
  "lat": 55.75,
  "lon": 37.62,
...
```
- Ошибки: `500` при сбое обработки модели
- Пример запроса:
```bash
curl -X POST http://localhost:1337/api/v1/proc_image \
...
- Метод: `POST /api/v1/crowd_analysis`
- Назначение: агрегирует результаты LLM по наборам изображений (фронтальная и дверные камеры) и возвращает итоговые метрики по салону
- Требования: в наборе обязательно должны быть фронтальные изображения (`cam_info = "frontal"`)
- Тело запроса (JSON):
```json
{
  "images": [
    { "cam_info": "frontal", "cam_num": 1, "gate_pos": [1, 2], "image_bytes": "<BASE64>" },
    { "cam_info": "gate",    "cam_num": 2, "gate_pos": 1,         "image_bytes": "<BASE64>" }
  ]
}
```
- Ответ 200 (JSON):
...
```
- Ошибки: `400` если не переданы фронтальные изображения
- Пример запроса:
```bash
curl -X POST http://localhost:1338/api/v1/crowd_analysis \
  -H 'Content-Type: application/json' \
  -d '{
    "images": [
      { "cam_info": "frontal", "cam_num": 1, "gate_pos": [1,2], "image_bytes": "<BASE64>" },
...
В [docker compose](./docker-compose.yaml) необходимо указать ссылку на провайдера и ключ API:
...
### Пример визуализации:
![Распределение людей в автобусе](./imgs/people_in_the_bus.png)

### Схема автобуса:
![Схема автобуса](./imgs/photo_2025-10-05_11-02-48.jpg)

### Фотография загруженного автобуса:
![loaded](./data/loaded_bus.jpg)

### Приммер интерфейса
![demo](./imgs/interface_demo.png)

### Пример настройки изображений
![demo_settings](./imgs/settings_demo.png)
